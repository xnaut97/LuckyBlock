...
public class LanguageManager {
	...
	private void setLanguage(Plugin plugin) {
		...
		if (newLanguage.isPresent() == false) {
			Bukkit.getLogger().severe("[EmpireX] Could not find language by locale tag '" + locale + "'");
	...
	public File getFile(@Nonnull Language language) {
		...
		if(file.exists() == false) throw new NullPointerException("There's not any language file called " + language.getLocale() + ".yml");
		return file;
	}
	 ...
	 *            The default language which will be returned in case the plugin has
	...
	private boolean validate(String section, String value) {
		if(value.contains("MemorySection[path='" + section + "', root='YamlConfiguration']"))
			return false;
		return true;
	}
}
